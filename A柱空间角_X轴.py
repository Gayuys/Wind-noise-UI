...
def visualize_a_pillar_3d(lower_point, upper_point,
    ...
    if angle_results is not None:
        info_text = (
            f"【核心输出】A柱与X轴夹角: {angle_results['direction_angles'][0]}°\n"
            f"A柱长度: {angle_results['a_pillar_length']} mm\n"
            f"与Y轴夹角: {angle_results['direction_angles'][1]}°\n"
    ...
    ax.set_title('A柱空间位置与角度分析（聚焦X轴夹角）', fontsize=14, pad=20)
...
def calculate(stl_path):
    ...
    if a_column_lower_point is not None and a_column_upper_point is not None:
        ...
        if angle_results is not None:
            # print("\n========================")
            # print(f"A柱与X轴的空间夹角: {angle_results['direction_angles'][0]}°")
            # print("========================")
            ...
            # )
            kongjiabnjiao_X = angle_results['direction_angles'][0]
        else:
