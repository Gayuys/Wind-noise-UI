...
def calculate(stl_path):
    ...
    # 计算X向差值的绝对值（后视镜X向尺寸）
    rearview_mirror_x_size = abs(x_max - x_min)

    # 打印后视镜X向尺寸结果
    #print(f"后视镜X向尺寸: {rearview_mirror_x_size:.2f}")

    # # 可视化部分（保留原逻辑，仅显示X向极值点）
    # fig, ax = plt.subplots()
    ...
    # ax.set_xlabel("X")
    # ax.set_ylabel("Y")
    # ax.set_title("Projected Points on XY Plane (X向尺寸)")
    # ax.axis("equal")
    # ax.grid(True)
    # plt.legend()
    # plt.show()

    return rearview_mirror_x_size
