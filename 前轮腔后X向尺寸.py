...
def get_back_point(points):
    """提取排序后的倒数第1、2个点（前轮腔后尺寸专用）"""
    if not points.size:
        return None, None
    points_list = points.tolist()
    sorted_points = sorted(points_list, key=lambda p: p[0])
    ...
    last1 = unique_points[-1] if len(unique_points) >= 1 else None
    last2 = unique_points[-2] if len(unique_points) >= 2 else None
    return last1, last2
...
def calculate(stl_path):
    ...
    last1, last2 = get_back_point(intersection_point3)
    if last1 is not None and last2 is not None:
        hou_size = abs(last1[0] - last2[0])
    ...
    #     plt.plot(last1[0], last1[2], 'o', color='r', markersize=5, label='点4')
    # if last2 is not None:
    #     plt.plot(last2[0], last2[2], 'o', color='b', markersize=5, label='点5')
    # plt.title('前轮腔后X向尺寸特征点')
    # plt.xlabel('X-axis')
    # plt.ylabel('Z-axis')
    # plt.axis('equal')
    # plt.grid(True)
