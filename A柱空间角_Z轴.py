...
def visualize_a_pillar_3d(lower_point, upper_point,
    ...
    if angle_results is not None:
        info_text = (
            f"【核心输出】A柱与Z轴夹角: {angle_results['direction_angles'][2]}°\n"
    ...
    ax.set_title('A柱空间位置与角度分析（聚焦Z轴夹角）', fontsize=14, pad=20)
...
def calculate(stl_path):
    ...
    if a_column_lower_point is not None and a_column_upper_point is not None:
        ...
        if angle_results is not None:
            # print("\n========================")
            # print(f"A柱与Z轴的空间夹角: {angle_results['direction_angles'][2]}°")
            # print("========================")
            ...
            # )
            kongjiabnjiao_Z = angle_results['direction_angles'][2]
        else:
