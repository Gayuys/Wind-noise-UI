    ...
    for n in range(3, len(sorted_points) + 1):
        ...
        if prev_angle is not None:
            ...
            if angle_diff > threshold:
                line_length = np.linalg.norm(np.array(best_points[-1][:2]) - np.array(best_points[0][:2]))
...
def calculate(stl_path):
    ...
    dian_3 = find_target_point1(contour_points3)

    # 下端R角计算和输出
    point_a = x_min_point
    point_b = dian_2
    r_angle = calculate_min_angle_with_y_axis(point_a, point_b)

    # 输出下端R角
    #print(f"下端R角：{r_angle:.2f}°")
    return r_angle
